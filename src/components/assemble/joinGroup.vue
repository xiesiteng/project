        ...
        <div class="ass-item" v-for="(item, index) in list" :key="index">
          ...
          <div class="item-right">
            ...
            <div class="price-wrap">
              ...
              <span class="pre-price">￥{{item.goods_price}}</span>
              <button class="ping" @click="toDetail(item.id)">去拼团</button>
            </div>
          </div>
        </div>
        <!--拼团end-->
...
<script>
  ...
  export default {
    ...
    methods: {
      init () {
        axios.get('/lan/group_list?group_type=' + this.active + '&page=' + this.page).then(this.initSucc).catch(err => console.log(err))
      },
      initSucc (res) {
        if (res.data.code == 2000) {
          ...
          this.endTime = this.list[0].end_time
          var date1 = new Date()
      ...
      toDetail (id) {
        this.$router.push({path: '/assemble/assemblePay', query: {goods_id: id}})
      },
