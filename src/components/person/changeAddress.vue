...
<script>
  ...
  export default {
    ...
    mounted () {
      ...
      this.name = this.$route.query.consignee
      ...
      // 保存原来的信息
      this.prename = this.$route.query.consignee
      this.prephone = this.$route.query.mobile
      this.preprovince = this.$route.query.province
      this.precity = this.$route.query.city
      this.predistrict = this.$route.query.district
      this.predetailAddress = this.$route.query.address
    },
    methods: {
      leave() {
      ...
      sureAdd () {
        if (this.name==this.prename || this.phone==this.prephone || this.province==this.preprovince || this.city == this.precity || this.district == this.predistrict || this.detailAddress == this.predetailAddress) {
          Toast('未进行任何修改')
          return false
        }
        var myreg = /^[1][3,4,5,7,8][0-9]{9}$/;
        ...
        axios.get('/lan/user_address_edit?consignee=' + this.name + '&mobile=' + this.phone + '&province=' + this.province + '&city=' + this.city + '&district=' + this.district + '&address=' + this.detailAddress + '&address_id=' + this.address_id).then(this.addSucc).catch(err => console.log(err))
      },
      addSucc (res) {
